...
export default function Home({ tokenId }) {
  ...
  const { mutateAsync, isLoading: isSettingTokenUri } = useContractWrite(contract, "setTokenURI");
          ...
          <Card
            ...
            body={
              <>
                ...
                />
                <button onClick={handleImageSubmit} disabled={!isConnected || selectedImageIdx === null || isUploadingImage}>
            ...
            body={
              <>
                <textarea
                  ...
                  rows={2}
                  style={{ borderColor: isExceedingCharLimit ? 'red' : 'inherit' }}
                />
                {isExceedingCharLimit && (
                  <p style={{ color: 'red' }}>Character limit exceeded (max: {promptLimit})</p>
                )}
                <button onClick={handlePromptSubmit} disabled={!isConnected || !prompt || isExceedingCharLimit || isLoadingImages}>
                  {!isConnected ? 'Connect to proceed' : isLoadingImages ? 'Generating...' : 'Generate'}
                </button>
              </>
            }
          />
